...
function initEmptyGraphState2(div) {
...
function deleteState(questionDiv, stateData) {
  ...
  if (!jeProhlizeciStranka_new()) {
    ...
    if (isEmpty(questionDiv)) {
      ...
      initEmptyGraphState2(questionDiv);
    }
  }
}
...
function createTableFromData(questionDiv) {
  ...
  MIN_STATE_WIDTH = findLongestTitle(table.states);
...
function insertRowHeader(row, name, cellWidth) {
  var cell = insertCell(row, row.cells.length, ["row-header-cell"], cellWidth);
  var table = findParentWithClass(cell, tableClasses.myTable);
  var input = createInput([tableClasses.inputCellDiv, tableClasses.rowHeader], name, name, cellWidth);
...
function insertRow(table, title) {
  ...
  if (tableVisualLength(title) > MIN_STATE_WIDTH) {
    MIN_STATE_WIDTH = tableVisualLength(title);
  }
...
function addResizable(table, cell, stateTitle = false) {
  $(cell).resizable({
    handles: 'e',
    resize: function () {
      ...
      if (parseInt(this.style.width) >= parseInt(minSize)) {
        ...
        for (var i = 1; i < table.rows.length - 1; i++) {
          ...
          t.rows[i].cells[ci].style.width = this.style.width;
